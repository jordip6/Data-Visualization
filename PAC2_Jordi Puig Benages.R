...
df_poblacio <- read.csv(file='Registre_central_de_poblaci__del_CatSalut__poblaci__per_municipi.csv')
colnames(df_poblacio)<- c('any','codi província','província','codi comarca','comarca','codi municipi','municipi','gènere','edat','poblacio')
...
poblacio_agrupada <- df_poblacio %>% group_by(any,grupEdat) %>% summarize(total_poblacio=sum(poblacio))
